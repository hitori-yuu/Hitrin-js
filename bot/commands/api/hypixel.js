...
const Errors = require('hypixel-api-reborn/src/Errors');
...
module.exports = {
	...
	async execute(interaction) {
		...
		const { hypixel } = interaction.client;
		...
		try {
			if (interaction.options.getSubcommand() === 'player') {
				hypixel.getPlayer(player, { guild: true }).then(player => {
					...
					if (error.message === '[hypixel-api-reborn] Player does not exist.') return CustomError(interaction, '存在しないプレイヤーです。');
					else return InteractionError(error, interaction);
				});
				...
				hypixel.getGuild('name', guild).then(guild => {
					...
					if (error.message === '[hypixel-api-reborn] Guild does not exist.') return CustomError(interaction, '存在しないギルドです。');
					else if (error.message === '[hypixel-api-reborn] Failed to connect.') return CustomError(interaction, '接続に失敗しました。');
					else return InteractionError(error, interaction);
				});
			}
		} catch (error) {
